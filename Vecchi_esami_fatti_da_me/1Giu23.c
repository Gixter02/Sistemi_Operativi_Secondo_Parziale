...
int main(int argc, char** argv) {
    ...
    char* store; /* variabile per leggere le linee del file associato */
    ...
    for(h = 0; h < H; h++){
        ...
        {	/* processo figlio */
            ...
            {	/* processo nipote */
                ...
                //int fd = 0; // variabile che conterra' il file descriptor del file argv[h+1]
            ...
            while (read(p[0], &store[j], sizeof(char)) > 0)	/* ciclo di lettura fino a che riesco a leggere un carattere da file */
            {
    ...
    for(int i = 0; i < H; i++){
        ...
        if ((status & 0xFF) != 0)
        {
            ...
            printf("Il filgio con PID = %d ha ritornato il valore %d", pid, ritorno);
        }
    }
